...
| 13 | answered iff `eventsObserved > 0 \|\| nonEventPackets > 0`; observation lists `eventTypes` and `nonEventPackets`, and says which half was answered (gate widened by ruling after this spec was drafted: this row first read `eventsObserved > 0` alone, but a stray non-event packet on a listening connection *is* the interleave half of the question, and §4.4 adds `nonEventPackets` to `Findings` for exactly that. The kit spec's §9a records the same amendment; `src/diagnostics/report.ts` is the shipped rule) | answered on a zero-event window |
...
| §4.2 library refusals | emulator `supportsBuffer: false` and legacy `USERTEMP_RRQ` answering `ACK_ERROR`, driven through `probeBulk`; users step `refused` with the deciding exchange's command 9 and ack 2001 (`test/diagnostics/probe.bulk.spec.ts`). As shipped this uses the emulator's existing `handlers` knob: the `refuseLegacyUsers` knob this row first named was never added, and the command numbers here were first written as 13/1503, which are `ATTLOG_RRQ` and `PREPARE_BUFFER`, not the refused user read | remove `classifyError`'s `refused` branch | `malformed`, command 9 and ack 2001 unchanged |
...
| §4.4 mid-window failure | emulator closes the socket 30 ms into a 60 s window (new knob `dropAfterRegisterMs`); `heldOpen === false`, item 9 answered "dropped", item 13 per events, and `endedAfterMs` bounded on a REAL injected clock — at least 20 ms, at most the probe call's own wall time, far short of the 60 000 ms window. That last part is stated here because the shipped test first injected the counting fake the other realtime tests use, which makes `endedAfterMs` 10 whatever the window did | no-op handler and unconditional sleep; and separately, force `endedAfterMs = 0` | held open, window full; and the lower bound fails |
