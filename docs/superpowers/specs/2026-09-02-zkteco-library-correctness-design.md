...
| §5.2 framing failure ends session | junk mid-stream during a request; the next call throws `ZkConnectionError` naming the closed session and the emulator saw EXIT | no abandon on `ZkFramingError` | the next call reaches the transport and rejects with the retained framing failure instead |
